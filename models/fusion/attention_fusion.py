...
class AttentionFusionModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.text_proj = nn.Linear(text_feat_dim, config.fusion_hidden_dim)
        
        # 图像编码器
        self.image_encoder = models.resnet50(pretrained=True)
        self.image_encoder = nn.Sequential(*list(self.image_encoder.children())[:-1])
        # 推断 image feature dim（ResNet50 -> 2048）
        image_feat_dim = getattr(config, "image_feature_dim", 2048)
        self.image_proj = nn.Linear(image_feat_dim, config.fusion_hidden_dim)
